async function yangshi2(dates) { 
    ...
    const bounds1 = wrapper.append("g").attr("id","bounds1")//画布里创建居中对齐的组
    ...
    .range([width*0.15, width*0.28])//dimensions.boundedRadius 直径
    .nice()
    ...
    const drawTimehour = (angle,offset,text)=>{//text
      const [x1, y1] = getCoordinatesForAngle(angle, offset)
      const [x2, y2] = getCoordinatesForAngle(angle, offset+0.1)  
    ...
    const areaHRGenerator = d3.areaRadial()//这个是外壳造型
      ...
      .innerRadius(/*d=>4*HRVRadiusScalexuxian(HRVAccessor(d))*/  d=>radiusScaleHR(HRbaseline/2))//定位
    ...
    for(i=0;i<datapath.length;i++){//我希望每一个两分钟的块都能突出来
      ...
      illustrate.selectAll(".round").data(datapath[i])
        ...
        .attr("d",
          d3.arc()
              .outerRadius(d=>{
                  if(d.SCRS == 0){return 0;}
                  else if((d.SCRS == 1)){return 0}
                  else if(d.SCRS == 2){return 0}
                  else if(d.SCRS != 0 && d.SDNN < baselinehrv && d.HR > HRbaseline){return timecircle+4}
                  else {return timecircle+2}             
              })
              .innerRadius(
                  d=>{
                    if(d.SCRS == 0){return 0;}
                    ...
                    else if(d.SCRS >= 3){return timecircle-4}
                    else {return timecircle-2}             
                  }
              )
          ...
          const pathes = g.selectAll(".biodata")
            ...
            .on("dblclick",(event,d)=>{
              const drawnoteline = (angle, offset) => {
                const [x1, y1] = getCoordinatesForAngle(angle, offset) 
                const [x3, y3] = getCoordinatesForAngle(angle, offset) 
                annotationGroup.append("line")
                  .attr("id","noteline")
                  .attr("class", "annotation-line-min")
                  .attr("stroke","black")
                ...
                annotationGroup.append("line")
                  ...
                  .attr("stroke-width",3)
                  .attr("x1", x3)
                  .attr("x2", d=>{if(x1>0){return x3+60}else{return x3-60}})
                  .attr("y1", y3)
                  .attr("y2", y3)
              }
                const noteline = drawnoteline(angleScale1(dateParseH(d.TIME.slice(-5))),0.7);
                drawtagline(angleScale1(dateParseH(d.TIME.slice(-5))),0.7);

            })

          //试一下倒影; 也许可以成line---------------------------------------------------------------------------------------------------------------------------------------------------------
            let gdaoying = lineGroup.append("g").datum(datapath[i])
            gdaoying.selectAll(".biodata-daoying").data(datapath[i]).join("path")
              .attr("class","biodata-daoying")
              .attr("stroke","none")
              .attr("d",
                d3.arc()
                .outerRadius(d=>radiusScaleHR(HRbaseline/3))
                ...
                .attr("fill",function(d) { 
                  ...
                  else if((d.SCRS >= 1 && d.SCRS < 5/*&& d.SDNN >= baselinehrv) || (d.SDNN >= baselinehrv && d.HR <= HRbaseline) || (d.SCRS == 0 && d.HR <= HRbaseline*/)){m = "lowstress"}
                  else if(d.SCRS >= 5 && d.SCRS < 10/*|| d.SDNN >= baselinehrv || d.HR <= HRbaseline*/){m = "middle"}
                  else if(d.SCRS >= 10 && d.SCRS <= 15/*&& d.SDNN < baselinehrv && d.HR > HRbaseline*/){m = "highhighstress"}
                  else {m = "highstress"}
                  return color1[m];

                } )
              .attr("opacity","1")
              .style("mask",'url(#maskmm)')//#maskmm
              .style("filter", "url(#blur)")
    ...
    d3.select("#date-display")
      .on("mousedown",(event,d)=>{            
        d3.selectAll("#biodata")
          ...
          .attr("opacity","0.8")
          .attr("fill", function(d) { 
            /*if(d.SCRS != 0){  return d3.hsl(100, 0, Math.abs(linearscr(d.SCRS))); }
            else{ return "#efefef" }*/
            return "grey";
          })
          .attr("d",
      ...
      d3.selectAll('.biodata')
        .on('mouseover', function(event,d) {
          ...
          const t = mous.style("opacity", 1).html("生理数据<br><br>"+"HR:"+d.outR+"<br>"+"HRV:"+d.SDNN+"<br>"+"SCR:"+d.SCRS)
          //显示时间刻度
      ...
      d3.selectAll('.biodata-daoying')
        ...
        .on('mouseout', function() {
          // 鼠标移出时，移除所有path元素上的"faded"类
          d3.selectAll('.biodata-daoying').classed('faded', false);
    ...
    //随着鼠标转的圆
      document.addEventListener('mousemove', function(event) {
        ...
        var radius = circleRect.width / 2 - 10; // 减去tracker的一半宽度以保持其完全在圆内
        ...
        trace.style.width = '5px';
        trace.style.height = '5px';
