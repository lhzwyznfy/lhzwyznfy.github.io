async function yangshi4(dates) {
  ...
  .style("transform", `translate(${
    dimensions.width/3-65
    }px, ${
    dimensions.height/3-20
    }px)`)
  //创建圆圈画布-----------------------------------------------------------------
  ...
  const tooltipheight ="8em";
  const tooltipwidth = "8em";
  tooltip.id="tooltip";
  ...
  tooltip.style.border="0.5px dashed grey";
  ...
  tooltip2.style("transform", `translate(${
    width-dimensions.margin.right-dimensions.boundedRadius/2-tooltip.clientWidth/2
    }px, ${
    dimensions.margin.top + dimensions.boundedRadius-tooltip.clientHeight/2
    }px)`)
  //tooltip--------------------------------------------------------------------
  ...
  //    
  const linearr = d3.scaleLinear().domain(d3.extent(dataobj1, TEMPAccessor)).range([0,0.02]);
  //
  ...
  const HRVRScale = d3.scaleSqrt()//平方根
    ...
    .range([10, 0.2])//50
  const EDARadiusScalexuxian = d3.scaleSqrt()//平方根
    .domain(d3.extent(dataobj1, EDAccessor))
    .range([0.2, 10])
      ...
      .attr("d",
        ...
        //HR，上面是外边框
          ...
          const pathes = g.selectAll(".biodata")
            ...
            .attr("d",
              d3.arc()
                .outerRadius(d=>radiusScaleHR(d.outR)+2*HRVRScale(d.SDNN)/2)
                .innerRadius(d=>radiusScaleHR(d.outR)-5*HRVRScale(d.SDNN))
                .startAngle(function(d) { return angleScale1(dateAccessorH(d)); })
                .endAngle(function(d) { return (angleScale1(dateAccessorH(d))+ 0.04); })
            )
            .attr("opacity","0.8")
            .attr("fill",function(d) { 
                if(d.SCRS == 0){m  = "lowlowstress"}
  ...
  .range([0, (vlows+lstres+mstress+hstress+vhighs)/1.2])
  .nice()
